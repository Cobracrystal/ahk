...
class AltDrag {
	...
	static scaleWindow(direction := 1, scale_factor := 1.025, wHandle := 0, overrideBlacklist := false, withMousePos := true) {
		...
		if (withMousePos && (mouseNonantH || mouseNonantV)) { ; if in middle nonant, use other method
			xChange := this.clamp(direction * xChange, wLimit.minW - winW, wLimit.maxW - winW)
			yChange := this.clamp(direction * yChange, wLimit.minH - winH, wLimit.maxH - winH)
			if (mouseNonantH == -1) {
				winW += xChange
			...
			xChange := this.clamp(2 * direction * xChange, wLimit.minW - winW, wLimit.maxW - winW)
			yChange := this.clamp(2 * direction * yChange, wLimit.minH - winH, wLimit.maxH - winH)
			winX -= xChange // 2
			winY -= yChange // 2
			winW += xChange
			winH += yChange
